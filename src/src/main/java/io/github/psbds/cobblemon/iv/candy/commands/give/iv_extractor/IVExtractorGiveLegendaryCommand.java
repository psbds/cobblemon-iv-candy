...
public class IVExtractorGiveLegendaryCommand {

    public static LiteralArgumentBuilder<CommandSourceStack> initialize() {
        return literal("legendary")
                .executes(IVExtractorGiveLegendaryCommand::handleGiveIVExtractorCommand);
    }
    ...
    private static int handleGiveIVExtractorCommand(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        for (ServerPlayer targetPlayer : targetPlayers) {
            var ivExtractor = IVExtractorFactory.createLegendary();
            PlayerInventoryHelper.addOrDrop(targetPlayer, ivExtractor);
            commandSender.sendSystemMessage(Component.literal("Gave Legendary IV Extractor to " + targetPlayer.getDisplayName().getString()));
        }
