...
public class CandyGiveUltraBeastCommand {

    public static LiteralArgumentBuilder<CommandSourceStack> initialize() {
        return literal("ultrabeast")
                .executes(CandyGiveUltraBeastCommand::handleGiveSpeciesCommand);
    }
    ...
    private static int handleGiveSpeciesCommand(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        for (ServerPlayer targetPlayer : targetPlayers) {
            ...
            var candy = CandyFactory.createUltraBeastCandy(targetIvStat);
            targetPlayer.getInventory().add(candy);
            commandSender.sendSystemMessage(Component.literal("Gave Ultra Beast IV Candy to " + targetPlayer.getDisplayName().getString()));
        }
