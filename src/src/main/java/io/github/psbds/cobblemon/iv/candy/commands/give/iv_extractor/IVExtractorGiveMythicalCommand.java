...
public class IVExtractorGiveMythicalCommand {

    public static LiteralArgumentBuilder<CommandSourceStack> initialize() {
        return literal("mythical")
                .executes(IVExtractorGiveMythicalCommand::handleGiveIVExtractorCommand);
    }
    ...
    private static int handleGiveIVExtractorCommand(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        for (ServerPlayer targetPlayer : targetPlayers) {
            var ivExtractor = IVExtractorFactory.createMythical();
            PlayerInventoryHelper.addOrDrop(targetPlayer, ivExtractor);
            commandSender.sendSystemMessage(Component.literal("Gave Mythical IV Extractor to " + targetPlayer.getDisplayName().getString()));
        }
