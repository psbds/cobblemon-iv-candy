...
public class ShardGiveUltraBeastCommand {

    public static LiteralArgumentBuilder<CommandSourceStack> initialize() {
        return literal("ultrabeast")
                .executes(ShardGiveUltraBeastCommand::handleGiveShardCommand);
    }
    ...
    private static int handleGiveShardCommand(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        for (ServerPlayer targetPlayer : targetPlayers) {
            var shard = ShardFactory.createUltraBeastShard();
            targetPlayer.getInventory().add(shard);
            commandSender.sendSystemMessage(Component.literal("Gave Ultra Beast IV Shard to " + targetPlayer.getDisplayName().getString()));
        }
