...
public class CandyGiveLegendaryCommand {

    public static LiteralArgumentBuilder<CommandSourceStack> initialize() {
        return literal("legendary")
                .executes(CandyGiveLegendaryCommand::handleGiveSpeciesCommand);
    }
    ...
    private static int handleGiveSpeciesCommand(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        for (ServerPlayer targetPlayer : targetPlayers) {
            ...
            var candy = CandyFactory.createLegendaryCandy(targetIvStat);
            targetPlayer.getInventory().add(candy);
            commandSender.sendSystemMessage(Component.literal("Gave Legendary IV Candy to " + targetPlayer.getDisplayName().getString()));
        }
